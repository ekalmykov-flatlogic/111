...
const CommentsTable = () => {
  ...
  const loading = useSelector((store) => store.comments.list.loading);
      ...
      <div>
        {null && (
          <Button
            color='default'
            size='xs'
            onClick={() => dispatch(push(`/admin/comments/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color='info'
          size='xs'
          onClick={() => history.push(`/admin/comments/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  const columns = [
...

    {
      ...
      formatter: articlesDataFormat.listFormatter,
  ...
  return (
    <div>
      <Widget title={<h4>Comments</h4>} collapse close>
        <div>
          <a
            href={
