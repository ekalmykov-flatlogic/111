...
const ArticlesTable = () => {
  ...
  const loading = useSelector((store) => store.articles.list.loading);
      ...
      <div>
        {null && (
          <Button
            color='default'
            size='xs'
            onClick={() => dispatch(push(`/admin/articles/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color='info'
          size='xs'
          onClick={() => history.push(`/admin/articles/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  return (
    <div>
      <Widget title={<h4>Articles</h4>} collapse close>
        <div>
          <a
            href={
